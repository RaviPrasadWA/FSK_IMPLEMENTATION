    ...
    int c;
        ...
        while ((c = fgetc(ctx->ch)) != EOF) {
            ...
            if ((ctx->sync && (c != LLP_ESC )) || (ctx->sync && (ctx->escape && (c == LLP_ESC || c == HDLC_FLAG || c == HDLC_RESET)))) {
                ...
                /* Check if we have read 12 bytes. If we have, we should now have a block of two data bytes 
                and a parity byte. This block*/
                if (ctx->readLength % LLP_INTERLEAVE_SIZE == 0) {
                    /*If the last character in the block looks like a control character, we need to set 
                    the escape indicator to false, since the next byte will be read immediately after the FEC
                    routine, and thus, the normal reading code will not reset the indicator.*/
                    if (c == LLP_ESC || c == HDLC_FLAG || c == HDLC_RESET) ctx->escape = false;
                    
                    // The block is interleaved, so we will first put the received bytes in the deinterleaving buffer
                    ...
                    // Adjust the packet length, since we will get parity bytes in the data buffer with block sizes larger than 3
                    ctx->frame_len -= LLP_INTERLEAVE_SIZE/3 - 1;

                    // For each 3-byte block in the deinterleaved bytes, we apply forward error correction
                    for (int i = 0; i < LLP_INTERLEAVE_SIZE; i+=3) {
                        // We now calculate a parity byte on the received data.
                        ...
                        /* By XORing the calculated parity byte with the received parity byte, we get
                        what is called the "syndrome". This number will tell us if we had any errors during 
                        transmission, and if so where they are. Using Hamming code, we can only detect 
                        single bit errors in a byte though, which is why we interleave the data, 
                        since most errors will usually occur in bursts of more than one bit. With 2 data byte 
                        interleaving we can correct 2 consecutive bit errors.*/
                        uint8_t syndrome = ctx->calculatedParity ^ p;
                        if (syndrome == 0x00) {
                            /* If the syndrome equals 0, we either don't have any errors, or the error is unrecoverable, 
                            so we don't do anything*/
                        } else {
                            /*If the syndrome is not equal to 0, there is a problem, and we will try to correct it. 
                            We first need to split the syndrome byte up into the two actual syndrome numbers, one for
                            each data byte.*/
                            uint8_t syndromes[2];
                            syndromes[0] = syndrome & 0x0f;
                            syndromes[1] = (syndrome & 0xf0) >> 4;

                            // Then we look at each syndrome number to determine what bit in the data bytes to correct.
                            for (int i = 0; i < 2; i++) {
                                uint8_t s = syndromes[i];
                                uint8_t correction = 0x00;
                                if (s == 1 || s == 2 || s == 4 || s == 8) {
                                    // This signifies an error in the parity block, so we actually don't need any correction
                                    continue;
                                }

                                // The following determines what bit to correct according to the syndrome value.
                                ...
                                // This is just for testing purposes. Nice to know when corrections were actually made.
                                if (s != 0) ctx->correctionsMade += 1;
                            }
                        }

                        // We now update the checksum of the packet with the deinterleaved and possibly corrected bytes.
...
/* This function calculates and returns a parity byte for two input bytes. The parity byte is
used for correcting errors in the transmission. The error correction algorithm is a standard 
(12,8) Hamming code.*/
...
/*Following is the functions responsible for interleaving and deinterleaving blocks of data. The interleaving table 
